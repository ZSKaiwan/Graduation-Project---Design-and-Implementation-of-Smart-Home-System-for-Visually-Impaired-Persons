...
/*********************功能函数*/
void OLED_Interface(void){          //配置界面函数
		//OLED_Clear();			
		OLED_ShowString(15, 0, "WIFI:", OLED_8X16);
		OLED_ShowChinese(25,16,"温度：");
		OLED_Printf(65, 16, OLED_8X16, "%2d.%1d C", temperature/10, temperature%10);
		OLED_ShowChinese(25,32,"湿度：");
		OLED_Printf(65, 32, OLED_8X16, "%2d.%1d RH", humidity/10, humidity%10);
		if(Start_flag == 1){
			OLED_ShowChinese(55,0,"连接成功");
		} 
		else{
			OLED_ShowChinese(55,0,"连接失败");
		}
	
		if(Window_Mode == 1){
			OLED_ShowChinese(25,48,"窗户：开");
		}
		else{
			OLED_ShowChinese(25,48,"窗户：关");
		}
		OLED_Update();
		// 屏幕稳定不闪烁
		Delay_ms(20);
