...
{
	DHT11_GPIO_Config();
	GPIO_SetBits(DHT11_Dout_GPIO_PORT,DHT11_Dout_GPIO_PIN);               // 拉高GPIOA2
	DHT11_Mode_IPU();
}
...
//数据采集
void DHTcollect_data(void){
		
		if(DHT11_Read_TempAndHumidity(&DHT11_Data) == SUCCESS)
		{
			// 把读到的值更新到全局变量
			temperature = DHT11_Data.temp_int * 10 + DHT11_Data.temp_deci;
			humidity    = DHT11_Data.humi_int * 10 + DHT11_Data.humi_deci;
			// 转换为浮点实际值，供MQTT上报使用
			temp = (float)temperature / 10.0f;
			humi = (float)humidity / 10.0f;
		}
		Delay_ms(500);
